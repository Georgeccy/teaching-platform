...
var DECK_KEY = 'xdf-yuban-u2-state';
var deckState = null;
...
// === Unit 2 custom drag-drop checkers (scoped per question, V9 §24) ===

// Q36 — Text 4 paragraph heading matching (课本 Part B1)
(function(){
  window.checkQ36Drag = function(){
    var zones = document.querySelectorAll('#q36-pool ~ div .drop-zone');
    var correct = 0, total = 7, placed = 0;
    zones.forEach(function(z){
    ...
    var res = document.getElementById('q36-result');
    if(res){
      res.innerHTML = (placed < total ? '⚠️ 还有 ' + (total - placed) + ' 个标题未拖入。' : (correct === total ? '✅ 全部正确！1-G · 2-F · 3-C · 4-A · 5-E · 6-D · 7-B' : '✅ ' + correct + ' / ' + total + ' 正确，再检查一下红色项。'));
      res.style.color = correct === total ? 'var(--fcc-green-dark)' : 'var(--brand-red)';
    }
    if(placed === total){ recordAnswer(correct === total); }
  };
  window.resetQ36Drag = function(){
    var pool = document.getElementById('q36-pool');
    document.querySelectorAll('#q36-pool .draggable, #q36-pool ~ div .drop-zone .draggable').forEach(function(d){
      d.classList.remove('correct-placed','wrong-placed');
      pool.appendChild(d);
    });
    sortDraggablesInPool(pool);
    var res = document.getElementById('q36-result'); if(res) res.innerHTML='';
  };
})();
