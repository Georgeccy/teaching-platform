...
var DECK_KEY = 'xdf-unit1-v3-state';
var deckState = null;
...
// === Random Number Picker (v3.3 — 随机点名) ===
var _randHistory = [];
...
function drawRandom() {
  ...
  _randRolling = setInterval(function() {
    numEl.textContent = 1 + Math.floor(Math.random() * max);
    ticks++;
    if (ticks >= total) {
      clearInterval(_randRolling); _randRolling = null;
      numEl.classList.remove('rolling');
      var finalNum = 1 + Math.floor(Math.random() * max);
...
function renderRandHistory() {
  ...
  if (!h) return;
  h.textContent = _randHistory.length ? ('记录：' + _randHistory.join(' · ')) : '';
}
document.addEventListener('keydown', function(e) {
  if (e.key === 'Escape') {
    var ov = document.getElementById('randOverlay');
...
// === Unit 1 custom drag-drop checkers (scoped per question, V9 §24) ===

// Q27 — Subheading matching (课本 Part B1)
(function(){
  window.checkQ27Drag = function(){
    var zones = document.querySelectorAll('#q27-pool ~ div .drop-zone');
    var correct = 0, total = 4, placed = 0;
    zones.forEach(function(z){
      var accept = z.dataset.accept;
      var item = z.querySelector('.drop-content .draggable');
      if(item){
        placed++;
        item.classList.remove('correct-placed','wrong-placed');
        if(item.dataset.cat === accept){ item.classList.add('correct-placed'); correct++; }
        else item.classList.add('wrong-placed');
      }
    });
    var res = document.getElementById('q27-result');
    if(res){
      res.innerHTML = (placed < total ? '⚠️ 还有 ' + (total - placed) + ' 个标题未拖入。' : (correct === total ? '✅ 全部正确！1-C · 2-D · 3-B · 4-A' : '✅ ' + correct + ' / ' + total + ' 正确，再检查一下红色项。'));
      res.style.color = correct === total ? 'var(--fcc-green-dark)' : 'var(--brand-red)';
    }
    if(placed === total){ recordAnswer(correct === total); }
  };
  window.resetQ27Drag = function(){
    var pool = document.getElementById('q27-pool');
    document.querySelectorAll('#q27-pool .draggable, #q27-pool ~ div .drop-zone .draggable').forEach(function(d){
      d.classList.remove('correct-placed','wrong-placed');
      pool.appendChild(d);
    });
    sortDraggablesInPool(pool);
    var res = document.getElementById('q27-result'); if(res) res.innerHTML='';
  };
})();

// SB Q12 — Title matching (Skill Builder)
(function(){
  window.checkSBQ12Drag = function(){
    var zones = document.querySelectorAll('#sbq12-pool ~ div .drop-zone');
    var correct = 0, total = 4, placed = 0;
    zones.forEach(function(z){
      var accept = z.dataset.accept;
      var item = z.querySelector('.drop-content .draggable');
    ...
    var res = document.getElementById('sbq12-result');
    if(res){
      res.innerHTML = (placed < total ? '⚠️ 还有 ' + (total - placed) + ' 个标题未拖入。' : (correct === total ? '✅ 全部正确！In two minds→1 · Encouragement→2 · Change→3 · Debt→4' : '✅ ' + correct + ' / ' + total + ' 正确，再检查一下红色项。'));
      res.style.color = correct === total ? 'var(--fcc-green-dark)' : 'var(--brand-red)';
    }
    if(placed === total){ recordAnswer(correct === total); }
  };
  window.resetSBQ12Drag = function(){
    var pool = document.getElementById('sbq12-pool');
    document.querySelectorAll('#sbq12-pool .draggable, #sbq12-pool ~ div .drop-zone .draggable').forEach(function(d){
      d.classList.remove('correct-placed','wrong-placed');
      pool.appendChild(d);
    });
    sortDraggablesInPool(pool);
    var res = document.getElementById('sbq12-result'); if(res) res.innerHTML='';
  };
})();
